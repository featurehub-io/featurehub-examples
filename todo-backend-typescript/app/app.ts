...
import { featureHubRepository, GoogleAnalyticsCollector, Readyness, StrategyAttributeCountryName, StrategyAttributeDeviceName, StrategyAttributePlatformName} from 'featurehub-repository/dist';
...
class TodoController implements ITodoApiController {
  ...
  async removeTodo(parameters: { id: string }): Promise<Array<Todo>> {
    featureHubRepository.logAnalyticsEvent('todo-remove', new Map([['gaValue', '5']]));
  ...
  async addTodo(parameters: { body?: Todo }): Promise<Array<Todo>> {
    featureHubRepository.logAnalyticsEvent('todo-add', new Map([['gaValue', '10']]));
    const todo: Todo = {
  ...
  private listTodos() : Array<Todo> {
    if (featureHubRepository.getFlag('FEATURE_TITLE_TO_UPPERCASE')) {
      const upperTodoList = [];
      todos.forEach((t) => {
        const newT = new Todo();
        newT.id = t.id;
